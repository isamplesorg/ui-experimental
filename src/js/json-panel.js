/**
 * Web component implementing a panel for displaying JSON
 */
...
export class JsonPanel extends LitElement {
...
`;
    ...
    /**
     * Given current traversal context, return a css class
     *
    ...
    jsonRecurse(obj, level, context) {
        ...
        if (typeof obj === "object") {
            if (Array.isArray(obj)) {
                for (let x in obj) {
                    ...
                    if (obj.hasOwnProperty(x)) {
                        ...
                        res.push(html`<span x-context="${_jp.toString()}" class="${s_class}">${this._indent(level + 1)}${this.jsonRecurse(obj[x], level + 1, context)}</span>`);
                    }
                }
                _x = context.pop();
                return html`[\n${this._tjoin(res, ",\n")}\n${this._indent(level)}]`;
                ...
                for (let x in obj) {
                    ...
                    if (obj.hasOwnProperty(x)) {
                        const _jp = new JsonPointer(context);
                        res.push(html`${this._indent(level + 1)}<span x-context="${_jp.toString()}" class="${s_class}">${JSON.stringify(x)}: ${this.jsonRecurse(obj[x], level + 1, context)}</span>`);
                    }
                }
                _x = context.pop();
                return html`{\n${this._tjoin(res, ",\n")}\n${this._indent(level)}}`;
            ...
            const _jp = new JsonPointer(context);
            let result = html`<span class="${v_class}" x-context="${_jp.toString()}" @click="${this._handleUriClick}">${v}</span>`;
            if (add_quotes) {
                result = html`"${result}"`;
            }
            _x = context.pop();
    ...
    render() {
        return html`<slot></slot><pre>${this.jsonToHtmlish(this.data)}</pre>
        `;
    }
